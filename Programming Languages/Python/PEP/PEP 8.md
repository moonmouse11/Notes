...
Используйте символы, отличные от `ASCII`, редко, желательно только для обозначения мест и имен людей. Если в качестве данных используются не `ASCII`-символы, избегайте неприятных символов Unicode, таких как `z̯̯͡a̧͎̺l̡͓̫g̹̲o̡̼̘` и знаки порядка байтов.
...
Импорты должны быть сгруппированы в следующем порядкe:
