...
***
## Check Existing
- `hasTable(string table_name): bool`  - метод для проверки существования таблицы.
- `hasColumn(string table_name, string field_name): bool` - метод для проверки существования поля.
- `hasColumns(stirng table_name, array fields_name): bool` - метод для проверки существования нескольких полей.
***
## Database changing
В Laravel есть два способа указать необходимую базу данных в миграции.
- `connection = string database_name` - свойство класса миграции. По умолчанию подставляется значение из файла конфигурации.
- `connection(string database_name)` - статический метод фасада `Schema`. Вызывается перед методами работы с таблицами.
``` php
// Пример использования свойства
protected $connection = 'pgsql';
// Пример использования статического метода
Schema::connection('pgsql')->create('table_name', function (Blueprint $table){
	// Код для работы с таблицей
});
```
***
